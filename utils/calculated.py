...
class calculated:
    ...
    def __init__(self, title=_("崩坏：星穹铁道"), platform=_("PC"), order="127.0.0.1:62001", adb_path="temp\\adb\\adb",
        ...
        # 初始化
        self.attack = cv.imread("./temp/pc/attack.png") if self.platform == _("PC") else cv.imread(
            "./temp/mnq/attack.jpg")
        ...
        self.finish = cv.imread("./temp/pc/finish_fighting.jpg") if self.platform == _("PC") else cv.imread(
    ...
    def click_target(self, target_path: str, threshold, flag: bool = True, check: bool = False):
        ...
        temp_ocr = {
            ...
            "map_3-5": _("丹鼎司"),
            "map_3-5_point_1":[(593, 500), (693, 700)],
            "map_3-5_point_5":[(600, 500), (600, 150)],
            "map_3-6": "(鳞渊境|鳞游渊境)",
            "map_3-6_point_1":[(688,431),(356,467)],
            "map_3-6_point_2":[(593, 500), (800, 700)]
        }
        if temp_name in temp_ocr:
            log.info(temp_name)
            if "orientation" in temp_name:
                log.info(_("选择星球") + temp_name)
            elif "point" in temp_name:
                log.info(_("选择传送锚点") + temp_name)
            elif "map" in temp_name:
                log.info(_("选择地图") + temp_name)
            if "map" not in temp_name:
                ...
                if type(temp_ocr[temp_name]) == str:
                    ...
                    first_timeout = True
                    while True:
                        ocr_data = self.part_ocr((77, 20, 85, 97)) if self.platform == _("PC") else self.part_ocr(
                            (72, 18, 80, 97))
                        log.debug(temp_ocr[temp_name])
                        check_dict = list(
